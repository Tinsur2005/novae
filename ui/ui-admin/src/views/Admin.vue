...
const handleStatusChange = async (row) => {
  try {
    const res = await request.put(`/admins/${row.id}`, {status: row.status})
    if (res.code === 1) {
  ...
  formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (form.id) {
        ...
        res = await request.put(`/admins/${form.id}`, data)
      } else {
        res = await request.post('/admins', form)
      }
      if (res.code === 1) {
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确定要删除用户「${row.name}」吗？`, '提示', {
    ...
    try {
      const res = await request.delete(`/admins/${row.id}`)
      if (res.code === 1) {
...
const handleDeleteBatch = () => {
  ElMessageBox.confirm(`确定要删除选中的 ${selection.value.length} 个用户吗？`, '提示', {
    ...
    try {
      ...
      const res = await request.delete('/admins', {data: ids})
      if (res.code === 1) {
