<script setup>
  ...
  const loadProducts = (sub) => {
    ...
    productApi.list({categoryId: sub.id, page: 1, limit: 10, status: 1}).then(result => {
      if (result.code === 1) {
    ...
    <div class="cat-body">
      ...
      <section class="cat-right">
        <template v-if="currentCategory">
          ...
          <van-empty v-else description="该分类下暂无子分类" image-size="60"/>
          ...
          <template v-if="activeSub">
            ...
            <van-empty v-else-if="!productLoading" description="该分类下暂无商品" image-size="60"/>
        ...
        <van-empty v-else description="分类加载中..."/>
      </section>
    </div>
  </div>
</template>
